...
@app.route('/search',methods=['POST','GET']) # route with allowed methods as POST and GET
def search():
    if request.method == 'POST':
            ...
            print(productLink,flush=True)
            prodRes = requests.get(productLink) # getting the product page from server
            prod_html = bs(prodRes.text, "html.parser") # parsing the product page as HTML
            commentboxes = prod_html.find_all('div', {'class': "_16PBlm _3_IKGE"}) # finding the HTML section containing the customer comments
